particionarSDA(){
	...
	parted -s /dev/sda mkpart primary ext4 0% 257MiB          #	256M    /boot	    	/dev/sda1 (tam. fixo)
	parted -s /dev/sda mkpart primary ext4 257MiB 3137MiB     #	8G      /     		  /dev/sda2 (tam. fixo)
	parted -s /dev/sda mkpart extended 3137MiB 4676MiB	      #	232G  	extended	  /dev/sda3
	  parted -s /dev/sda mkpart logical ext4 3395MiB 3651MiB	#	2G	    linux-swap	/dev/sda5 (tam. fixo)
